...
class PneumaticTunerApp:
    def __init__(self, root):
        ...
        if self.df_logs is not None:
            print(f"Колонки в ЛОГАХ: {list(self.df_logs.columns)}")

        self.df_registry = dm.load_database(self.REGISTRY_FILE)
        self.df_reference_setpoint = dm.load_database(self.REFERENCE_SETPOINT_FILE)

        if self.df_logs is None or self.df_registry is None or self.df_reference_setpoint is None:
            messagebox.showerror("Ошибка", "База данных (Логи, Реестр или Настройки) не найдена!")
    ...
    def setup_layout(self):
        ...
        labels = self.df_registry['cycle_id'].dropna().unique().tolist()
        ...
        # Добавляем в список отображения target-колонку
        # display_sensors = self.SENSORS_LIST + ["p_tank_target", "p_tank_theory"]
        display_sensors = self.SENSORS_LIST
        self.sensor_controls = []
        for s in display_sensors:
            cfg = self.gui_config["sensors"].get(s, {"visible": False, "color": "blue", "ls": "-"})
            # Если это целевое давление - пусть по умолчанию будет красным пунктиром
            if s == "p_tank_target" and s not in self.gui_config["sensors"]:
                cfg = {"visible": True, "color": "red", "ls": "--"}
    ...
    def update_view(self, *args):
        ...
        df_plot = self.df_logs[self.df_logs['cycle_id'] == current_id].sort_values('t_relative')
        
        if not df_plot.empty:
            # 8. Вызов отрисовки
            self.plotter.draw(df_plot, self.gui_config, current_id)

        # 9. Сохранение конфига UI
