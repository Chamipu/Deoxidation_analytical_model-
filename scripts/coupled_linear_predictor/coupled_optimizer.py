...
OPTIMIZATION_T_MAX = 10.0

# Настройки глобального алгоритма Дифференциальной Эволюции
DE_SETTINGS = {
    'strategy': 'best1bin',      # Стратегия выбора кандидатов и скрещивания
    'maxiter': 50,              # Максимальное количество поколений (итераций поиска)
    'popsize': 10,               # Множитель размера популяции (размер = popsize * число параметров)
    'tol': 0.01,                 # Критерий сходимости (остановка при малом изменении ошибки)
    'mutation': (0.5, 1.0),      # Масштаб мутации (широта охвата пространства поиска)
    'recombination': 0.7,        # Вероятность скрещивания параметров (кроссовер)
    'polish': False,              # Локальный спуск в конце для сверхточной доводки параметров
