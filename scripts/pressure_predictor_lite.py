...
def apply_analytic_model(df_logs, df_registry, config, params):
    # 1. Извлекаем данные
    times = df_logs[config['col_time']].values
    # targets = df_logs[config['col_target_tank']].values
    targets = df_logs[config['col_target_tank_rounded']].values
    ...
    for i in range(len(times)):
        ...
        k_gain = (params['k_gain']*p_current)+params['b_gain']
        target = _get_delayed_target(i, times, targets, params['dead_time'])
    ...
    df_logs[config['col_result_tank']] = predictions

    df_logs, df_registry = _calculate_and_save_errors(df_logs, df_registry, config)
...
def _calculate_and_save_errors(df_logs, df_registry, config):
    """
    ...
    df_logs[config['col_MAE_tank']] = (df_logs["IBA_DB\PT1009 Актуальное давление в баке P1 (бар)_median"] - df_logs[config['col_result_tank']]).abs()
    ...
    cycle_errors = df_logs[
        (df_logs[config['col_time']] >= 6) & (df_logs[config['col_time']] <= 15)
    ].groupby(config['col_cycle'])[config['col_MAE_tank']].mean()

    # 3. Переносим значения из полученной серии в df_registry
    # map сопоставит cycle_id в регистре с ключами в cycle_errors
    df_registry[config['col_MAE_tank']] = df_registry[config['col_cycle']].map(cycle_errors)
